...
const HEADER = ['ID', '日時', 'iPad', '残したいデータ', 'Windowsのパスワード', '心当たり', 'お困りの症状', '受信日時'];
...
function doPost(e) {
  ...
  try {
    ...
    const row = [r.id, r.at, r.device, r.data, r.pcpass, r.change, r.symptom, new Date()].map(v => v == null ? '' : String(v));
